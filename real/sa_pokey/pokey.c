...
void Pokey_PutByte(UWORD addr, UBYTE byte) {
    ...
    case _AUDCTL:
        ...
        Update_Counter((1 << CHAN1) | (1 << CHAN2) | (1 << CHAN3) |
                       (1 << CHAN4));
        Update_pokey_sound(_AUDCTL, byte, 0, SOUND_GAIN);
        break;
    case _AUDF1:
        AUDF[CHAN1] = byte;
        Update_Counter((AUDCTL[0] & CH1_CH2) ? ((1 << CHAN2) | (1 << CHAN1))
                       : (1 << CHAN1));
    ...
    case _AUDF3:
        AUDF[CHAN3] = byte;
        Update_Counter((AUDCTL[0] & CH3_CH4) ? ((1 << CHAN4) | (1 << CHAN3))
                       : (1 << CHAN3));
    ...
    if (chan_mask & (1 << CHAN2)) {
        ...
        if (AUDCTL[0] & CH1_CH2) {
            if (AUDCTL[0] & CH1_179)
                ...
                DivNMax[CHAN2] =
                    (AUDF[CHAN2] * 256 + AUDF[CHAN1] + 1) * Base_mult[0];
        } else
    ...
    if (chan_mask & (1 << CHAN4)) {
        ...
        if (AUDCTL[0] & CH3_CH4) {
            if (AUDCTL[0] & CH3_179)
                ...
                DivNMax[CHAN4] =
                    (AUDF[CHAN4] * 256 + AUDF[CHAN3] + 1) * Base_mult[0];
        } else
