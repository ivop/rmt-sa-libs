...
int main(int argc, char **argv) {
    ...
    printf("Number of instruments: %i, start at $%04x\n", ninstruments,
           itable);
    printf("Number of tracks: %i, lsb's start at $%04x, " "msb's at $%04x\n",
    ...
    for (i = 0; i < ninstruments; i++) {
        ...
        printf("\tTABLE OF NOTES\n\t\ttlen: $%02x, tgo: $%02x, "
               ...
               (p[4] & 0x3f) + 1, p[4] & 0x80 ? "freqs" : "notes",
               p[4] & 0x40 ? "accum" : "add");
        printf("\t\t");
        for (c = 12; c <= tend; c++) {
            printf("%02x ", p[c]);
    ...
    for (p = mem + song, i = 0; p < mem + end[0] + 1; i++) {
        printf("%02x: ", i);
        if (p[0] == 0xfe) {
            printf("goto line --> %02x",
                   ((p[2] + (p[3] << 8)) - song) / channels);
            if (channels == 8)
                p += 4;
        } else {
